...
(function() {
	...
	(function() {
		...
		var uEvent=function() {
			...
			noFS&&curCfg.winAlpha!=1&&setStyle(ym_win,isIE?{}:{opacity:1});	//鼠标按下时取消窗体的透明度
		...
		addEvent('mousedown',function(e) {
			...
			e = e || window.event;
			/*frameset下不能实现半透明否则会消失，原因不明*/
			noFS&&curCfg.winAlpha!=1&&setStyle(ym_win,isIE?{}:{opacity:curCfg.winAlpha});//鼠标按下时窗体的透明度
			ymPrompt.apply(dragVar, {
		...
		var setWinTitle = function(titleStr) {
			if (!ym_win || ym_win.style.display == 'none') return;
				curCfg.title = titleStr;
				ym_hText.innerHTML = titleStr;
		};
